...
namespace Order.UnitTests.Features.Orders.Commands.MarkStockReserved;

public class MarkStockReservedCommandHandlerTests
{
    private readonly Mock<IApplicationDbContext> _mockDbContext;
    private readonly Correlation _correlation;
    private readonly MarkStockReservedCommandHandler _handler;

    public MarkStockReservedCommandHandlerTests()
    {
        _mockDbContext = new Mock<IApplicationDbContext>();
        _correlation = new Correlation { Id = Guid.NewGuid() };
        _handler = new MarkStockReservedCommandHandler(_mockDbContext.Object, _correlation);
    }

    [Fact]
    public async Task Handle_WithExistingOrder_ShouldMarkReservedAndConfirm()
    {
        // Arrange
        var order = CreateTestOrder();
        order.Submit(_correlation.Id.ToString());
        var orderId = order.Id;
        var command = new MarkStockReservedCommand(orderId);
    ...
    public async Task Handle_WithNonExistingOrder_ShouldThrowNotFoundException()
    {
        ...
        var command = new MarkStockReservedCommand(nonExistingId);
    ...
    public async Task Handle_ShouldCallSaveChangesAsync()
    {
        ...
        var order = CreateTestOrder();
        order.Submit(_correlation.Id.ToString());
        var command = new MarkStockReservedCommand(order.Id);
